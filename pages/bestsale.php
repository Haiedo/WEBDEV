...
<div class="main">
    ...
    while ($row_content = mysqli_fetch_array($query_bdm)) {
        ...
        $sql_bsp = "SELECT * FROM bang_san_pham WHERE bang_san_pham.id_danh_muc = $thu_tu_id AND tinh_trang = 1 ORDER BY id_san_pham DESC";
        $query_bsp = mysqli_query($conn, $sql_bsp);
    ?>
    <h2 class="title_content">DANH MỤC <?php echo mb_strtoupper($row_content['ten_danh_muc'])?> BÁN CHẠY</h2>
    <div class="main-content">
        <div class="products">
            <?php
            while ($row_pro = mysqli_fetch_array($query_bsp)) {
                if ($row_pro['so_luong'] < 20) {
            ?>
            <div class="product">
